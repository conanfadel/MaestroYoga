...
- `backend/app/web_ui/` - صفحات الويب: `impl_state/` (حزمة: `_bindings` للاستيرادات المشتركة + `_constants`، `_guards_redirects`، …)، `public_routes.py` (يجمع `public_*`؛ تسجيل/دخول: `public_auth_register` + `public_auth_login_logout` ← `public_auth_register_login`؛ حساب عام: `public_auth_account_profile` + `public_auth_account_delete` ← `public_auth_account`؛ توثيق البريد: `public_auth_verify_pending_routes` + `public_auth_verify_actions_routes` ← `public_auth_verify`؛ حجز/سلة: `public_commerce_book_routes` + `public_commerce_cart_routes` ← `public_commerce_routes`؛ تصفح/أخبار: `public_browse_utils_routes` (إصدار محتوى) + `public_browse_news_routes` ← `public_browse_routes`؛ `public_feedback_routes`)، `admin_routes.py` (يجمع `admin_auth`، `admin_dashboard`، `admin_org_*` بما فيها `admin_org_plans_routes` و`admin_org_faqs_routes`، `admin_center`)، `admin_dashboard_context.py` مع `admin_dashboard_context_load` و`admin_dashboard_context_compose`، و`admin_dashboard_blocks.py` (بناء سياق `GET /admin`: غرف/جلسات/مستخدمين/مدفوعات/منشورات، KPI، أمن، ترقيم)، `admin_reports_html.py` (تقارير HTML؛ الجلسات في `admin_reports_html_sessions`؛ الإيراد في `admin_reports_html_revenue` مع `admin_reports_html_revenue_data`؛ برميل `admin_reports_html_sessions_revenue`؛ `insights` مع `admin_reports_html_insights_data`؛ `rest` يجمّع `clients`، `subscriptions`، `health`، `security_audit`، `center_report_settings`)، `admin_reports_attendance_summary` مع `admin_reports_attendance_summary_data` (حضور جلسة، ملخص طباعة/بريد)، و`admin_reports_*` الأخرى، و`impl.py` يجمّع الـ `router`؛ التصدير من `__init__.py`
...
- **Apple Pay:** إن كان لديك تكامل **Apple Pay** برقم منفصل، عيّن `PAYMOB_APPLE_PAY_INTEGRATION_ID` و`PAYMOB_USE_APPLE_PAY_INTEGRATION=1` ليُستخدم هذا الرقم في `payment_keys` (صفحة الدفع المستضافة). انسخ **Iframe Link** الخاص بتكامل Apple Pay إلى `PAYMOB_IFRAME_LINK` إن اختلف عن رابط البطاقة. أكمل في لوحة Paymob أي خطوات (نطاق، شهادات) يطلبها الدعم. يمكن الإبقاء على `PAYMOB_INTEGRATION_ID` للبطاقة في البيئة ثم تعطيل `PAYMOB_USE_APPLE_PAY_INTEGRATION` للعودة للبطاقة.
- **Iframe Link (مُفضّل إن وُجد في اللوحة):** انسخ **رابط واجهة الدفع** (`Iframe Link`) كاملاً بصيغة `https://...` **من دون** معامل `payment_token` (احذف `?payment_token=` إن كان فارغاً). ضعه في **`PAYMOB_IFRAME_LINK`** أو **`PAYMOB_IFRAME_CHECKOUT_BASE`**. بعد إنشاء الطلب، يُلحق الخادم تلقائياً `payment_token=...` الصادر من API. يمكنك أيضاً لصق وسم `<iframe src="...">` وسيُستخرج الرابط منه.
- **Iframe ID (بديل):** إن لم تلصق الرابط، استخدم `PAYMOB_IFRAME_ID` من **Developers** → **iframes**، أو اتركه فارغاً فيُستخدم مسار افتراضي بنفس `PAYMOB_INTEGRATION_ID`.
