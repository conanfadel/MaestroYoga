...
- `backend/app/web_ui/` - صفحات الويب: `impl_state.py`، `public_routes.py` (يجمع وحدات `public_*`)، `admin_routes.py` (يجمع `admin_auth`، `admin_dashboard`، `admin_org_*`، `admin_center`)، `admin_dashboard_context.py` مع `admin_dashboard_context_load` و`admin_dashboard_context_compose`، و`admin_dashboard_blocks.py` (بناء سياق `GET /admin`: غرف/جلسات/مستخدمين/مدفوعات/منشورات، KPI، أمن، ترقيم)، `admin_reports_html.py` (تقارير HTML؛ الجلسات/الإيراد في `admin_reports_html_sessions` و`admin_reports_html_revenue` وبرميل `admin_reports_html_sessions_revenue`؛ `insights` مع `admin_reports_html_insights_data`؛ `rest` يجمّع `clients`، `subscriptions`، `health`، `security_audit`، `center_report_settings`) و`admin_reports_*` الأخرى، و`impl.py` يجمّع الـ `router`؛ التصدير من `__init__.py`
