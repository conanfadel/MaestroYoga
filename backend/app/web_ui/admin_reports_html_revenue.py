"""Admin HTML report: revenue, VAT, methods, and pending payment aging (route + data layer)."""
...
from .admin_reports_html_revenue_data import build_revenue_report_template_context


def register_admin_report_html_revenue_routes(router: APIRouter) -> None:
    ...
    def admin_report_revenue(
        ...
        ctx = build_revenue_report_template_context(
            db=db,
            cid=cid,
            center=center,
            d0=d0,
            ...
            date_from=date_from,
            date_to=date_to,
            admin_qp_payment_date_from=core.ADMIN_QP_PAYMENT_DATE_FROM,
            admin_qp_payment_date_to=core.ADMIN_QP_PAYMENT_DATE_TO,
        )
        return core.templates.TemplateResponse(request, "admin_report_revenue.html", ctx)
